...
function acopf_solve(opfmodel, opf_data, Pg, Qg, Va, Vm)
  ...
  opfmodel[:Va] = Va0
  @show size(Pg0)
  set_start_value.(Pg, Pg0)
  set_start_value.(Qg, Qg0)
  set_start_value.(Vm, Vm0)
  set_start_value.(Va, Va0)
...
function acopf_model(opf_data)
  ...
  # JuMP model now
  #
  opfmodel = Model(with_optimizer(alm.Optimizer))
  ...
  @NLconstraint(opfmodel, Va[opf_data.bus_ref] <= buses[opf_data.bus_ref].Va)
  @NLconstraint(opfmodel, Va[opf_data.bus_ref] >= buses[opf_data.bus_ref].Va)
  # @show opf_data.bus_ref
  # @show Va[opf_data.bus_ref]
  # @show buses[opf_data.bus_ref].Va
...
#		 )
  ...
  #
  for i in 1:ngen
    @NLconstraint(opfmodel, Pg[i] <= generators[i].Pmax)
    @NLconstraint(opfmodel, generators[i].Pmin <= Pg[i])

    @NLconstraint(opfmodel, generators[i].Qmin <= Qg[i])
    @NLconstraint(opfmodel, Qg[i] <= generators[i].Qmax)
  end

  # @NLconstraint(opfmodel, generators[i].Pmin <= Pg[i=1:ngen] <= generators[i].Pmax)
  # @NLconstraint(opfmodel, generators[i].Qmin <= Qg[i=1:ngen] <= generators[i].Qmax)
  # @NLconstraint(opfmodel, buses[i].Vmin <= Vm[i=1:nbus] <= buses[i].Vmax)
